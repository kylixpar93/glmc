...
create or replace package body dev_cpm_prepay_pkg is
  ...
  begin
        ...
        FROM dev.dev_cpm_api_data j,
             JSON_TABLE(j.json_data,
                        ...
                        COLUMNS(supplierNum VARCHAR2(500) PATH
                                ...
                                segment11 varchar2(200) path '$.segment11')) t
    ...
    SELECT COUNT(*)
     ...
     WHERE fc.ENABLED_FLAG = 'Y'
       AND fc.CURRENCY_CODE = l_invoiceCur;
    IF l_invoiceCur IS NULL OR v_check_cur = 0 THEN
    ...
    if l_empCode is null or lengthb(l_empCode) > 50 then
      ...
      l_return_detail := 'Mã nhân viên tối đa 50 ký tự! Vui lòng kiểm tra lại tại CPM!';
      x_status        := l_return_status;
      x_mess          := l_return_mess;
      x_detail        := l_return_detail;
      raise I_Exception;
    ...
    if lengthb(l_empManager) > 50 and l_empManager is not null then
      ...
      l_return_detail := 'Mã cán bộ quản lý tối đa 50 ký tự! Vui lòng kiểm tra lại tại CPM!';
      x_status        := l_return_status;
      x_mess          := l_return_mess;
      x_detail        := l_return_detail;
      raise I_Exception;
    ...
    if lengthb(l_contract) > 30 and l_contract is not null then
      ...
      l_return_detail := 'Số hợp đồng tối đa 30 ký tự! Vui lòng kiểm tra lại tại CPM!';
      x_status        := l_return_status;
      x_mess          := l_return_mess;
      x_detail        := l_return_detail;
      raise I_Exception;
    ...
    if lengthb(l_docNum) > 120 and l_docNum is not null then
      ...
      l_return_mess   := 'Lỗi tại Số chứng từ!';
      l_return_detail := 'Số chứng từ tối đa 120 ký tự! Vui lòng kiểm tra lại tại CPM!';
      x_status        := l_return_status;
      x_mess          := l_return_mess;
      x_detail        := l_return_detail;
      raise I_Exception;
    ...
    if lengthb(l_project) > 150 and l_project is not null then
      ...
      l_return_detail := 'Công trình tối đa 150 ký tự! Vui lòng kiểm tra lại tại CPM!';
      x_status        := l_return_status;
      x_mess          := l_return_mess;
      x_detail        := l_return_detail;
      raise I_Exception;
    ...
    if l_checkEmpType = 0 then
      ...
      l_return_detail := 'Đối tượng tạm ứng không có trên Oracle GL! Vui lòng kiểm tra lại tại CPM!';
      x_status        := l_return_status;
      x_mess          := l_return_mess;
      x_detail        := l_return_detail;
      raise I_Exception;
    ...
    if l_docNumCpm is null then
      ...
      l_return_detail := 'Số chứng từ tham chiếu CPM không được để trống! Bắt buộc truyền dữ liệu';
      x_status        := l_return_status;
      x_mess          := l_return_mess;
      x_detail        := l_return_detail;
      raise I_Exception;
      ...
      if l_checkDocNumCpm <> 0 then
        ...
        l_return_mess   := 'Lỗi tại Số chứng từ tham chiếu!';
        l_return_detail := 'Số chứng từ tham chiếu CPM đã tồn tại trên Oracle GL! Vui lòng kiểm tra lại tại CPM';
        x_status        := l_return_status;
        x_mess          := l_return_mess;
        x_detail        := l_return_detail;
        raise I_Exception;
    ...
    if l_segment3 is not null then
      ...
      if l_check_segment = 0 then
        ...
        l_return_detail := 'Phòng ban không tồn tại trên Oracle GL! Vui lòng kiểm tra lại dữ liệu COA Department tại CPM';
        x_status        := l_return_status;
        x_mess          := l_return_mess;
        x_detail        := l_return_detail;
        raise I_Exception;
    ...
    if l_segment11 is not null then
      select count(*)
        into l_check_segment
       ...
       where nvl(a.attribute1, 'x') =
         ...
         and a.flex_value = l_segment11;
      if l_check_segment = 0 then
        l_return_status := 'E22PREB030';
        l_return_mess   := 'Lỗi tại Bộ phận!';
        l_return_detail := 'Bộ phận không tồn tại trên Oracle GL! Vui lòng kiểm tra lại dữ liệu COA Unit tại CPM';
        x_status        := l_return_status;
        x_mess          := l_return_mess;
        x_detail        := l_return_detail;
        raise I_Exception;
      else
        v_segment11 := l_segment11;
      end if;
    else
      v_segment11 := c_segment11;
    end if;
    begin
    ...
    if v_bank_account_num is null then
      ...
      if v_bank_account_id is null then
        ...
        l_return_mess   := 'Lỗi tại Tài khoản ngân hàng!';
        l_return_detail := 'Tài khoản ngân hàng chi tiền không tồn tại trên Oracle GL. Kiểm tra lại số tài khoản ngân hàng!';
        x_status        := l_return_status;
        x_mess          := l_return_mess;
        x_detail        := l_return_detail;
        raise I_Exception;
        ...
        if v_bank_acct_use_id is null then
          ...
          l_return_mess   := 'Lỗi tại Tài khoản ngân hàng!';
          l_return_detail := 'Tài khoản ngân hàng chi tiền chưa được gán' ||
                             ' ở OU ' || l_org_id ||
                             ' Vui lòng kiểm tra lại tại Oracle GL!';
          x_status        := l_return_status;
          x_mess          := l_return_mess;
          x_detail        := l_return_detail;
          raise I_Exception;
  ...
  PROCEDURE create_invoice_header(p_data_id     NUMBER,
    ...
    l_sql                  varchar2(4000);
    v_doc_sequence_num     number;
    v_doc_category_code    varchar2(400);
    --l_checkEmpType   number;
    l_docNumCpm varchar2(200);
    --l_checkDocNumCpm number;
    l_org_id         NUMBER;
  ...
  BEGIN
    ...
    l_baseAmount := l_invoiceAmount * nvl(l_exRate, 1);
    Mo_Global.Init('SQLAP');
    ...
    IF x_status = 'S' THEN
      ...
      IF l_invoice_id IS NOT NULL THEN
        ...
        x_detail := 'Invoice num: ' || v_invoice_num;
      END IF;
  ...
  BEGIN
    x_status := 'S';
    Mo_Global.Init('SQLAP');
    Prc_Gen_Org(l_org_id, l_org_code, v_sob_id, v_Legal_Entity_Id);
    begin
      /*SELECT frv.responsibility_id
       into v_resp_id
       FROM apps.fnd_profile_options_vl       fpo,
    ...
    BEGIN
      SELECT t.supplierNum
        ...
        FROM dev.dev_cpm_api_data j,
             JSON_TABLE(j.json_data,
                        '$.Invoice' COLUMNS(supplierNum VARCHAR2(500) PATH
                                '$.supplierNum')) t
       WHERE j.data_id = p_data_id;
    EXCEPTION
      WHEN OTHERS THEN
    ...
    END;
    BEGIN
      SELECT ap.VENDOR_ID, ap.PARTY_ID
        INTO l_vendor_id, l_party_id
        FROM ap_suppliers ap
       WHERE ap.ATTRIBUTE13 = l_supplierNum;
    EXCEPTION
      WHEN OTHERS THEN
    ...
    END;
    begin
      select site.VENDOR_SITE_ID, site.ACCTS_PAY_CODE_COMBINATION_ID
        into v_VENDOR_SITE_ID, v_liablity_cc_id
        from ap_supplier_sites_all site
       where site.VENDOR_ID = l_vendor_id
         and upper(site.VENDOR_SITE_CODE) like 'T%';
    exception
    ...
    BEGIN
      SELECT ai.BATCH_ID,
             ...
             ai.SET_OF_BOOKS_ID,
             ai.ORG_ID
        INTO v_batch_id, v_invoice_amount, v_vendor_id, v_Sob_Id, l_org_id
        FROM ap_invoices_all ai
    ...
    FOR rec IN (SELECT upper(Type) invoiceType,
      ...
      IF X_status = 'S' THEN
        ...
        COMMIT;
        x_mess   := 'Tạo dữ liệu invoice line thành công!';
        x_detail := 'Tạo dữ liệu invoice line thành công!';
      END IF;
  ...
  procedure create_payment(p_data_id    number,
    ...
    v_invoice_type         varchar2(200);
    --v_entity_id            number;
    v_address1 varchar2(400);
    v_address2 varchar2(400);
    v_address3 varchar2(400);
    v_address4 varchar2(400);
    v_city     varchar2(100);
    v_state    varchar2(400);
    v_zip      varchar2(200);
    v_province varchar2(400);
  ...
  BEGIN
    ...
    IF l_invalid_json = 0 THEN
      ...
      l_return_detail := 'Kiểm tra lại tagname, cấu trúc json đã truyền từ CPM!';
      x_status        := l_return_status;
      x_mess          := l_return_mess;
      x_detail        := l_return_detail;
      raise l_Exception;
    ...
    if l_username is null then
      ...
      if l_user_id is null then
        ...
        l_return_mess   := 'Lỗi tại Người tạo chứng từ!';
        l_return_detail := 'Người tạo chưa được khai báo hoặc đã hết hiệu lực. Kiểm tra tại CPM và Oracle GL!';
        x_status        := l_return_status;
        x_mess          := l_return_mess;
        x_detail        := l_return_detail;
        raise l_Exception;
    ...
    x_detail := 'Invoice num: ' || l_invoice_num || ', Check number: ' ||
                l_payment_number;
    dev_cpm_api_pkg.update_data_api(p_data_id       => p_data_id,
                                    p_return_status => x_status,
                                    p_return_mess   => x_mess,
                                    p_return_detail => x_detail);
  exception
