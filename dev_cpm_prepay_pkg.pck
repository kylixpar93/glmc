...
create or replace package body dev_cpm_prepay_pkg is
    ...
    if v_bank_account_num is null then
      ...
      if v_bank_account_id is null then
        ...
        if v_bank_acct_use_id is null then
          ...
          l_return_mess   := 'Lỗi tại Tài khoản ngân hàng!';
          l_return_detail := 'Tài khoản ngân hàng chi tiền chưa được gán' || ' ở OU ' || l_org_id ||
  ...
  begin
    ...
    exception
      when others then
        ...
        l_return_detail := 'Lỗi hệ thống! Không tạo được Payment. Vui lòng liên hệ IT! ' || sqlerrm;
        x_status        := l_return_status;
        x_mess          := l_return_mess;
        x_detail        := l_return_detail;
        raise v_invalid;
    ...
    if v_payamount < 0 then
      ...
      exception
        when others then
          ...
          l_return_detail := 'Lỗi hệ thống. Không tạo được history payment. Vui lòng liên hệ IT!  ' || sqlerrm;
          x_status        := l_return_status;
          x_mess          := l_return_mess;
          x_detail        := l_return_detail;
          raise v_invalid;
  ...
  BEGIN
    ...
    x_detail := 'Invoice num: ' || l_invoice_num ||', Check number: '||l_payment_number;
