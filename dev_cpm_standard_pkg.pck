...
create or replace package body dev_cpm_standard_pkg is
  ...
  begin
      ...
      select isApply
        ...
        FROM dev.dev_cpm_api_data j,
             JSON_TABLE(j.json_data,
                        '$' COLUMNS(isApply varchar2(1) path '$.isApply')) t
        ...
        FROM dev.dev_cpm_api_data j,
             JSON_TABLE(j.json_data,
                        ...
                        COLUMNS(supplierNum VARCHAR2(500) PATH
                                ...
                                docNumCpm NUMBER path '$.docNumCpm',
    ...
    if l_isApply is null then
      l_return_status := 'E22PREB004';
      l_return_mess   := 'isApply không được bỏ trống!';
      l_return_detail := 'isApply bắt buộc nhập Y/N!';
      x_status        := l_return_status;
      x_mess          := l_return_mess;
      x_detail        := l_return_detail;
      raise I_Exception;
      ...
      if l_isApply = 'Y' then
        ...
        if nvl(l_invoiceAmount, 0) < nvl(l_applyAmount, 0) then
          l_return_status := 'E22PREB004';
          l_return_mess   := 'Lỗi số tiền Apply!';
        ...
        for pre in (select applyDate, applyPaymentNum
                      from dev.dev_cpm_api_data j,
                           JSON_TABLE(j.json_data,
                                      '$.Invoice'
                                      COLUMNS(NESTED PATH
          ...
          if pre.applydate is null or
            ...
            l_return_status := 'E22PREB004';
            l_return_mess   := 'Lỗi ngày apply!';
            l_return_detail := 'Ngày apply không được để trống và phải đúng định dạng DD-MM-YYYY!';
            x_status        := l_return_status;
            x_mess          := l_return_mess;
            x_detail        := l_return_detail;
            raise I_Exception;
          ...
          select count(*)
           ...
           where ai.INVOICE_NUM = pre.applypaymentnum;
          if l_check_preNum = 0 then
            l_return_status := 'E22PREB004';
            l_return_mess   := 'Lỗi số hóa đơn apply!';
            l_return_detail := 'Số hóa đơn apply không được để trống hoặc không có trên hệ thống!';
            x_status        := l_return_status;
            x_mess          := l_return_mess;
            x_detail        := l_return_detail;
            raise I_Exception;
          ...
          if l_pre_vendor_id <> l_vendor_id then
            l_return_status := 'E22PREB004';
            l_return_mess   := 'Lỗi nhà cung cấp PrePayment!';
            l_return_detail := 'Nhà cung cấp Prepayment không trùng với nhà cung cấp Standard';
            x_status        := l_return_status;
            x_mess          := l_return_mess;
            x_detail        := l_return_detail;
            raise I_Exception;
    ...
    if l_type is null or l_type <> 'STANDARD' then
      ...
      l_return_detail := 'Phân loại chứng từ phải là Standard!';
      x_status        := l_return_status;
      x_mess          := l_return_mess;
      x_detail        := l_return_detail;
      raise I_Exception;
    ...
    END IF;
    BEGIN
      SELECT SUM(t.amountLine) line_amount, t.invoiceAmount
        ...
        FROM dev.dev_cpm_api_data j,
             JSON_TABLE(j.json_data,
                        ...
                        COLUMNS(invoiceAmount NUMBER path '$.invoiceAmount',
                                ...
                                COLUMNS(amountLine NUMBER PATH '$.amountLine'))) t
       WHERE j.data_id = p_data_id
    ...
    if v_isPayment is null then
      ...
      l_return_mess   := 'Lỗi payment không được để trống!';
      l_return_detail := 'isPayment không được để trống!';
      x_status        := l_return_status;
      x_mess          := l_return_mess;
      x_detail        := l_return_detail;
      raise I_Exception;
          ...
          if v_bank_account_id is null then
            ...
            if v_doc_sequence_id is null then
              ...
              l_return_detail := 'Không tồn tại sequence ' ||
                                 v_sequence_name || ' Vui lòng Liên hệ IT';
              x_status        := l_return_status;
              x_mess          := l_return_mess;
              x_detail        := l_return_detail;
              raise I_Exception;
    ...
    FOR rec IN (SELECT upper(Type) invoiceType,
      ...
      l_concat_seg := rec.segment1 || '.' || rec.segment2 || '.' ||
                      rec.segment3 || '.' || rec.segment4 || '.' ||
                      rec.segment5 || '.' || rec.segment6 || '.' ||
    ...
    for journal in (select giftInvoice,
                      ...
                      FROM dev.dev_cpm_api_data j,
                           JSON_TABLE(j.json_data,
                                      '$' COLUMNS(giftInvoice VARCHAR2(500) PATH
                                              ...
                                              segment3 varchar2(200) PATH
                                              '$.Journal.segment3',
                                              segment4Dr varchar2(200) path
                                              '$.Journal.segment4Dr',
                                              segment4Cr varchar2(200) path
                                              '$.Journal.segment4Cr',
                                              segment11 varchar2(200) path
                                              '$.Journal.segment11',
                                              amount varchar2(200) path
                                              '$.Journal.amount',
      ...
      if journal.giftinvoice = 'Y' then
        ...
        if journal.segment4dr is null then
          l_return_status := 'E22PREB030';
          l_return_mess   := 'Tài khoản nợ không được để trống!';
          l_return_detail := 'Tài khoản nợ không được để trống!';
          x_status        := l_return_status;
          x_mess          := l_return_mess;
          x_detail        := l_return_detail;
          raise I_Exception;
          ...
          select count(*)
           ...
           where flv.FLEX_VALUE_SET_ID = ffs.FLEX_VALUE_SET_ID
             ...
             and flv.FLEX_VALUE = journal.segment4Dr;
          if l_check_segment = 0 then
            l_return_status := 'E22PREB030';
            l_return_mess   := 'Tài khoản nợ không tồn tại trên Oracle GL!';
            l_return_detail := 'Tài khoản nợ không tồn tại trên Oracle GL!';
            x_status        := l_return_status;
            x_mess          := l_return_mess;
            x_detail        := l_return_detail;
            raise I_Exception;
        ...
        if journal.segment4cr is null then
          l_return_status := 'E22PREB030';
          l_return_mess   := 'Tài khoản có không được để trống!';
          l_return_detail := 'Tài khoản có không được để trống!';
          x_status        := l_return_status;
          x_mess          := l_return_mess;
          x_detail        := l_return_detail;
          raise I_Exception;
          ...
          select count(*)
           ...
           where flv.FLEX_VALUE_SET_ID = ffs.FLEX_VALUE_SET_ID
             and ffs.FLEX_VALUE_SET_NAME = 'MF_COA_ACCOUNT'
             and flv.FLEX_VALUE = journal.segment4Cr;
          if l_check_segment = 0 then
            l_return_status := 'E22PREB030';
            l_return_mess   := 'Tài khoản có không tồn tại trên Oracle GL!';
            l_return_detail := 'Tài khoản có không tồn tại trên Oracle GL!';
            x_status        := l_return_status;
            x_mess          := l_return_mess;
            x_detail        := l_return_detail;
            raise I_Exception;
        ...
        l_concat_seg := journal.segment1 || '.' || journal.segment2 || '.' ||
                        ...
                        v_segment5 || '.' || journal.segment4dr || '.' ||
                        v_segment6 || '.' || v_segment7 || '.' ||
                        v_segment8 || '.' || v_segment9 || '.' ||
                        v_segment10;
        dev_cpm_api_pkg.check_ccid(p_coa_id     => l_coa_id,
                                   p_concat_seg => l_concat_seg,
        ...
        if journal.amount is null or
           dev_cpm_api_pkg.is_number(journal.amount) = 'N' then
          l_return_status := 'E22PREB030';
          l_return_mess   := 'Số tiền không được để trống!';
          l_return_detail := 'Số tiền phải có định dạng số!';
          x_status        := l_return_status;
          x_mess          := l_return_mess;
          x_detail        := l_return_detail;
          raise I_Exception;
        ...
        if journal.budget is not null then
          ...
          if l_check_segment = 0 then
            l_return_status := 'E22PREB030';
            l_return_mess   := 'Ngân sách không tồn tại trên Oracle GL!';
            l_return_detail := 'Ngân sách không tồn tại trên Oracle GL!';
            x_status        := l_return_status;
            x_mess          := l_return_mess;
            x_detail        := l_return_detail;
            raise I_Exception;
        ...
        if journal.campaign is not null then
          ...
          if l_check_segment = 0 then
            l_return_status := 'E22PREB030';
            l_return_mess   := 'Mã chiến dịch không tồn tại trên Oracle GL!';
        ...
        if journal.fromdate is not null and
          ...
          l_return_status := 'E22PREB030';
          l_return_mess   := 'Chi phí phát sinh từ ngày không được để trống!';
          l_return_detail := 'Chi phí phát sinh từ ngày định dạng DD-MM-YYYY!';
          x_status        := l_return_status;
          x_mess          := l_return_mess;
          x_detail        := l_return_detail;
          raise I_Exception;
        ...
        if journal.todate is not null and
          ...
          l_return_status := 'E22PREB030';
          l_return_mess   := 'Chi phí phát sinh đến ngày không được để trống!';
          l_return_detail := 'Chi phí phát sinh đến ngày định dạng DD-MM-YYYY!';
          x_status        := l_return_status;
          x_mess          := l_return_mess;
          x_detail        := l_return_detail;
          raise I_Exception;
        ...
        if journal.app is not null then
          ...
          if l_check_segment = 0 then
            l_return_status := 'E22PREB030';
            l_return_mess   := 'Mã ứng dụng không tồn tại trên Oracle GL!';
        ...
        if journal.partner is not null then
          ...
          if l_check_segment = 0 then
            l_return_status := 'E22PREB030';
            l_return_mess   := 'Mã đối tác hợp tác không tồn tại trên Oracle GL!';
        ...
        if journal.customer is not null then
          select count(*)
            ...
            from fnd_flex_values flv, fnd_flex_value_sets ffs
           where flv.FLEX_VALUE_SET_ID = ffs.FLEX_VALUE_SET_ID
             and ffs.FLEX_VALUE_SET_NAME = 'MF_Customer'
             and flv.FLEX_VALUE = journal.customer;
          if l_check_segment = 0 then
            l_return_status := 'E22PREB030';
            l_return_mess   := 'Mã khách hàng không tồn tại trên Oracle GL!';
        ...
        if journal.employee is not null then
          select count(*)
            ...
            from fnd_flex_values flv, fnd_flex_value_sets ffs
           where flv.FLEX_VALUE_SET_ID = ffs.FLEX_VALUE_SET_ID
             and ffs.FLEX_VALUE_SET_NAME = 'MF_Employee'
             and flv.FLEX_VALUE = journal.employee;
          if l_check_segment = 0 then
            l_return_status := 'E22PREB030';
            l_return_mess   := 'Mã nhân viên không tồn tại trên Oracle GL!';
  ...
  BEGIN
    ...
    FOR rec IN (SELECT upper(Type) invoiceType,
      ...
      v_concat_seg := v_segment1 || '.' || v_segment2 || '.' || v_segment3 || '.' ||
                      v_segment11 || '.' || v_segment5 || '.' || v_segment4 || '.' ||
  ...
  begin
    ...
    for pre in (select applyAmount,
      ...
      if l_Result_Boolean = false then
        l_return_Status := 'E22STAB002';
        l_return_mess   := l_Error_message;
  ...
  procedure import_journal(p_data_id    number,
                           ...
                           x_mess       out varchar2,
                           x_detail     out varchar2) is
    v_ledger_name            varchar2(200) := 'MC_LEDGER';
    ERR                      Exception;
    l_num                    number;
    ...
    v_average_journal_flag gl_je_batches.AVERAGE_JOURNAL_FLAG%type;
    ...
    v_segment8    varchar2(30);
    v_segment9    varchar2(30);
    v_segment10   varchar2(30);
  begin
    begin
      select frv.RESPONSIBILITY_ID
        into v_resp_id
    ...
    select gcc.SEGMENT5,
      ...
      into v_segment5,
           ...
           v_segment10
      from gl_code_combinations gcc
     where gcc.CODE_COMBINATION_ID = v_ccid;
    ...
    for journal in (select giftInvoice,
      ...
      l_num        := 1;
      v_concat_seg := journal.segment1 || '.' || journal.segment2 || '.' ||
                      journal.segment3 || '.' || journal.segment11 || '.' ||
                      v_segment5 || '.' || journal.segment4dr || '.' ||
                      v_segment6 || '.' || v_segment7 || '.' || v_segment8 || '.' ||
                      v_segment9 || '.' || v_segment10;
  ...
  BEGIN
    ...
    begin
      select isApply, giftInvoice, isPayment
        ...
        FROM dev.dev_cpm_api_data j,
             JSON_TABLE(j.json_data,
                        '$' COLUMNS(isApply varchar2(1) path '$.isApply',
                                    isPayment varchar2(1) path '$.isPayment',
  ...
  if l_return_status <> 'S' then
    ...
    if l_return_status = 'S' then
      ...
      x_detail := 'Invoice num: ' || l_invoice_num ||case when l_isPayment = 'Y' then ', Payment num: ' ||
                  l_payment_number end ||case when l_giftInvoice = 'Y' then ', Journal Doc Num: '||l_document end;
